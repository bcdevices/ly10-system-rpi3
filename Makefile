...
install-nerves-bootstrap:
	cd /tmp &&  \
	mix archive.install git https://github.com/nerves-project/nerves_bootstrap.git tag v1.6.1 --force
...
dist: dist-prep build
	[ -d $(ARTIFACT_DIR) ] && \
		fakeroot MIX_TARGET=$(MIX_TARGET) mix nerves.artifact $(PRJTAG) --path $(DIST) \
