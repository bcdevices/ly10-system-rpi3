...
install-nerves-bootstrap:
	cd /tmp &&  \
	mix archive.install git https://github.com/nerves-project/nerves_bootstrap.git tag v1.0.0 --force
...
docker: clean
	...
	-docker rm -f "$(PRJTAG)"
	docker run --name "$(PRJTAG)" -v $$HOME/.nerves/dl:/root/.nerves/dl -t "bcdevices/$(PRJTAG)" /bin/bash -c 'MIX_TARGET=ly10_rpi3 make dist'
